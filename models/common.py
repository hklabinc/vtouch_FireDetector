...
class DetectMultiBackend(nn.Module):
    ...
    def __init__(self, weights='yolov5s.pt', device=torch.device('cpu'), dnn=False, data=None, fp16=False, fuse=True):
        ...
        from models.experimental import attempt_download, attempt_load_v5  # scoped to avoid circular import
        ...
        if pt:  # PyTorch
            model = attempt_load_v5(weights if isinstance(weights, list) else w, device=device, inplace=True, fuse=fuse)
    ...
    def forward(self, im, augment=False, visualize=False):
        ...
        if self.pt:  # PyTorch
            ...
            raise NotImplementedError(f'ERROR: not a supported format')
