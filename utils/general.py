...
import pandas as pd
import torch
import torch.nn.functional as F     # added by hhchoi
import torchvision
import yaml
...
from utils.augmentations import classify_transforms, letterbox  # added by hhchoi
from utils.general_v5 import Profile
from utils.torch_utils_v5 import time_sync    # added by hhchoi
...
# added by hhchoi
def check_img_size_v5(imgsz, s=32, floor=0):
...
def apply_classifier(x, model, img, im0, imgsz, names, device):
    ...
    for i, d in enumerate(x):  # per image
        if d is not None and len(d):
            ...
            for j, a in enumerate(d):  # per item
                ...
                im = cv2.resize(cutout, imgsz)  # BGR
            ...
            t0 = time.time()
            pred_cls2 = []
            for j, imj in enumerate(ims):  # per box image
                # Run inference
                dt = (Profile(), Profile(), Profile())
                with dt[0]:                        
                    imj = torch.Tensor(imj).to(device)
                    imj = imj.half() if fp16 else imj.float()  # uint8 to fp16/32
                    if len(imj.shape) == 3:
                        imj = imj[None]  # expand for batch dim
            ...
            x[i] = x[i][torch.tensor(np.array(pred_cls2))]  # retain matching class detections
